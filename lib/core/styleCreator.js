...
const initialSdades = [1, 2, 3, 4, 5, 6, 7, 8, 9];
...
const createRoot = (theme, passedShades) => {
  const shades = passedShades || initialSdades;
  ...
  colorNames.forEach((color) => {
    if (Object.keys(theme).indexOf(color) !== -1) {
      ...
      shades.forEach((shade) => {
        root += `--${color}-d${shade}: ${colorHandler(
          shade * -0.1,
          theme[color]
        )};`;
        root += `--${color}-l${shade}: ${colorHandler(
          shade * 0.1,
          theme[color]
        )};`;
      });
...
const createClass = (theme, options) => {
  const shades = options.shades || initialSdades;
  ...
  if (options.transition) {
    styleText += `[class*='${prefixText}bg-'],[class*='${prefixText}text-'],[class*='${prefixText}border-'],[class*='${prefixText}fill-'] {transition: ${options.transition}s;}`;
  }

  colorNames.forEach((color) => {
    if (Object.keys(theme).indexOf(color) !== -1) {
      // add base colors
      styleText += `.${prefixText}text-${color}{color:var(--${color})}`;
      styleText += `.${prefixText}text-${color}-dark{color:var(--${color}-d3)}`;
      styleText += `.${prefixText}text-${color}-light{color:var(--${color}-l3)}`;
      styleText += `.${prefixText}bg-${color}{background-color:var(--${color})}`;
      styleText += `.${prefixText}bg-${color}-dark{background-color:var(--${color}-d3)}`;
      styleText += `.${prefixText}bg-${color}-light{background-color:var(--${color}-l3)}`;
      styleText += `.${prefixText}border-${color}{border-color:var(--${color})}`;
      styleText += `.${prefixText}border-${color}-dark{border-color:var(--${color}-d3)}`;
      styleText += `.${prefixText}border-${color}-light{border-color:var(--${color}-l3)}`;
      styleText += `.${prefixText}fill-${color}{fill:var(--${color})}`;
      styleText += `.${prefixText}fill-${color}-dark{fill:var(--${color}-d3)}`;
      styleText += `.${prefixText}fill-${color}-light{fill:var(--${color}-l3)}`;

      // add shades
      shades.forEach((shade) => {
        styleText += `.${prefixText}text-${color}-d${shade}{color: var(--${color}-d${shade})}`;
        styleText += `.${prefixText}text-${color}-l${shade}{color: var(--${color}-l${shade})}`;
        styleText += `.${prefixText}bg-${color}-d${shade}{background-color: var(--${color}-d${shade})}`;
        styleText += `.${prefixText}bg-${color}-l${shade}{background-color: var(--${color}-l${shade})}`;
        styleText += `.${prefixText}border-${color}-d${shade}{border-color: var(--${color}-d${shade})}`;
        styleText += `.${prefixText}border-${color}-l${shade}{border-color: var(--${color}-l${shade})}`;
        styleText += `.${prefixText}fill-${color}-d${shade}{fill: var(--${color}-d${shade})}`;
        styleText += `.${prefixText}fill-${color}-l${shade}{fill: var(--${color}-l${shade})}`;
      });
    }
  });
